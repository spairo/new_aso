(function wrapper() {
	...
	angular
	.module('app')
	.controller('DetailBlotterWarrantsController', DetailBlotterWarrantsController);

	function DetailBlotterWarrantsController(detailBlotterWarrantsService, $cookies, $location, $route, $routeParams, $timeout) {
		// Functions
		...
		function onSubmitClone(resp){
			if(resp.data.id){
				$location.path('/blotter-warrants/detalle-emision/1/' + resp.data.id);
			}
		}
		...
		this.cloneDefault =  function cloneDefault(objt) {
			...
			this.submitEmission = function submitEmission() {
				...
				if (this.openModalStatus) {
					...
					// Si se entro al modal de registro de histórico
					if (this.estatus) {
			...
			detailBlotterWarrantsService // Complejidad
			.getComplexity()
		...
		DetailBlotterWarrantsController.prototype = {
